...
Often, dynamic programming can be split into 3 steps: the base cases, the recurrence, and the answer. 

## Base Cases

The base cases are the cases of our dynamic programming that cannot be defined in terms of other cases of the same function. Typically, we set these cases manually before performing our reccurence in order to ensure that our recurrence works properly.

## Recurrence/Transition

The recurrence of our dynamic programming is the way we define each case in the terms of other cases of the same function. We always want the recurrence to be made up of cases that we have computed before our current case. 

## The Answer

Lastly, we want to figure out which case of our dynamic programming is the answer to our problem. This is usually an intuitive step of the dynamic programming and will almost always come naturally. 

## Solution - The $N$th Fibonacci Term Problem

A common introductory problem for dynamic programming is finding the N-th Fibonacci term. This can be done with recursion with the formula $F_n$ = $F_{n-1}$ + $F_{n-2}$ in $O(N^2)$ by simply recursing down to 0 for each term. However, notice that we are repeating some steps over and over for each term. In order to reduce our complexity, we can use dynamic programming. 

Let $\texttt{dp}[x]$ be the x-th term in the fibonacci sequence. We can use our recursive formula for our recurrence:

$$ \texttt{dp}[N] = \texttt{dp}[N-1] + \texttt{dp}[N-2]

However, notice that for the cases $N = 1$ and $N = 0$, there doesnt exist a \texttt{dp}[N-2]! These 2 cases are our base cases. For this problem, $N = 1$ and $N = 0$ are our only base cases, and we can simply set \texttt{dp}[0] $ = 0$ and \texttt{dp}[1] $ = 1$.

Lastly, we need to figure out which case of our dynamic programming is our answer. Since we defined $\texttt{dp}[x]$ as the $x$th term in the fibonacci sequence and we want to find the $N$th term, we simply take \texttt{dp}[N] as our answer.
