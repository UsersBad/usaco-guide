...
export class Problem {
      ...
      if (autoGeneratedSolURL === null) {
        for (let i = 1994; i <= 2017; ++i) {
          ...
          if (this.name.indexOf(des) != -1) {
            let num = i - 1994 + 20;
            autoGeneratedSolURL =
              `https://ioinformatics.org/page/ioi-2010/` + num.toString();
            break;
          }
        }
      }
  ...
  constructor(
    ...
    if (!solID) {
      ...
      let source = 'CPH';
      let cphUrl = getSec(source, books[source][0], solID.substring(4));
      this.solution = {
        kind: 'link',
